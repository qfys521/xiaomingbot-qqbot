...
- `intents`: 事件订阅数组，程序启动时会自动解析并转换为底层的位掩码。下面是全部可选的 Intent 名称及其说明：

  | Intent 名称              | 说明                                                   | 适用场景               |
  |-------------------------|--------------------------------------------------------|----------------------|
  | `GROUP_AND_C2C_EVENT`   | 群聊（包含 @机器人及无 @消息）与 C2C 单聊消息事件接收        | ✅ **QQ 群聊/私聊机器人** |
  | `GUILDS`                | 公域大频道内创建、更新、删除等基础管理事件                 | 频道机器人             |
  | `GUILD_MEMBERS`         | 频道内成员进出变动事件                                    | 频道机器人             |
  | `GUILD_MESSAGES`        | 频道内全部消息（含普通发言，需要**私域特权**配置）            | 私域频道机器人         |
  | `GUILD_AT_MESSAGES`     | 频道内他人 @ 机器人发话消息                               | 公域频道机器人         |
  | `INTERACTION`           | 机器人发出内嵌按钮、菜单时，用户点击所产生交互回调事件        | 按钮与卡片交互场景      |
  | `PUBLIC_GUILD_MESSAGES` | 用户在群聊里 @机器人或者发送一对一私聊事件（老版本开放平台意图）| 兼容场景               |
